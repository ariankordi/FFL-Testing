...
void RootTask::prepare_()
{
    ...
    {
        fprintf(stderr, "FFLInitResEx() failed with result: %d\n", (s32)result);
#if RIO_RELEASE
    ...
    // Set projection matrix
    {
        ...
        gWindowAspect = f32(window->getWidth()) / f32(window->getHeight());
        // Calculate the field of view (fovy) based on the given parameters
        f32 fovy = 2 * atan2f(43.2f / gWindowAspect, 500.0f);
...
void RootTask::createModel_(char (*buf)[FFLICHARINFO_SIZE])
{
    ...
    if (result != FFL_RESULT_OK)
    {
        RIO_LOG("input is not FFLStoreData (failed result %d), treating as FFLiCharInfo\n", result);
...
void RootTask::calc_()
{
    ...
    // Zoom out a bit if the view isn't square.
    if (gWindowAspect != 1.0f)
        radius += gWindowAspect * 200.0f;
